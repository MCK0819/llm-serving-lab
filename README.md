...
**중단 후 복구할 수 있는 문서 처리 Worker까지 구현했습니다.** 14개 구현 단계 중 1–4단계와 6–8단계, 총 7개를 완료했습니다. 기본 API, 스트리밍 제한·연결 정리, API Key 인증, 조직별 문서 접근, PDF 텍스트 추출·청크·CPU 임베딩과 비동기 처리·재시도를 구현했습니다. 실제 Redis·Celery Worker의 강제 종료 후 복구도 검증했습니다. 원격 GPU 검증은 보류하고 GPU 없이 가능한 문서 단계를 먼저 진행했습니다. RAG 답변 연결·관측·성능 측정은 아직 남아 있습니다. 질문 경로는 인증 후 RAG 미준비 상태를 반환하며, 성공 스트림은 시험용 데이터로 검증했습니다. 아래 구성은 전체 목표입니다.
