...
**RAG 연결에 요청 횟수 제한·안전한 종료·민감정보 로그 보호까지 구현했습니다.** 14개 구현 단계 중 1–4단계와 6–10단계, 총 9개를 완료했습니다. 기본 API, 스트리밍 제한·연결 정리, API Key 인증, 조직별 문서 접근, PDF 텍스트 추출·청크·CPU 임베딩, Worker 중단 복구와 RAG 파이프라인을 구현했습니다. 질문은 같은 조직의 문서만 검색하며, 모델에 전달한 근거와 응답의 출처 목록을 맞춥니다. 실제 PostgreSQL과 시험용 HTTP 모델 응답으로 연결을 검증했고, 실제 Qwen 토크나이저로 입력 한도를 확인했습니다. 원격 GPU 생성과 답변 품질 검증, 실제 배포·백업 복원, 관측·성능 측정은 아직 남아 있습니다. 아래 구성은 전체 목표입니다.

10단계 최종 검증은 로컬 253개, Linux/PostgreSQL 316개 통과입니다. 별도 실제 Docker 종료 두 경우와 Worker 복구 두 경우도 통과했습니다. [제한과 종료 검증 기록](docs/verification/service-limits.md)에 시험 조건과 한계를 정리했습니다.
