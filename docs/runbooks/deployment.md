...
처음 진행한다면 [사용자와 함께하는 첫 GPU 검증](first-gpu-session.md)의 순서대로 가입·접속 준비·비용 승인을 진행한다. 가입 이후에도 이미지 준비와 견적 승인 전에는 유료 Pod를 생성하지 않는다.
...
- 사용할 사용자 정의 이미지의 시작 도구와 SSH 연결이 검증되어 있다. `deploy/Dockerfile.vllm-ssh`의 전체 로컬 빌드와 실제 SSH 터널 검증을 마쳤다. 레지스트리 게시와 원격 GPU 기동은 남아 있으며, GPU 대여 전 게시된 이미지 주소를 확정해야 한다.
...
저장소 루트에서 `docker build -f deploy/Dockerfile.vllm-ssh -t <REGISTRY_IMAGE>:gpu-preflight .`로 파생 이미지를 빌드한다. 로컬 `llm-serving-lab:vllm-ssh-preflight` 빌드는 완료했다. 레지스트리 위치와 공개 범위를 정하고 게시한 뒤 최종 digest를 환경 기록에 추가한다. 게시는 아직 수행하지 않았다.

전용 `.dockerignore`는 SSH 설정과 시작 도구만 이미지에 포함한다. 키는 이미지에 넣지 않고 Pod 환경변수 `LLM_LAB_SSH_PUBLIC_KEY`에 공개키 한 개를 제공한다. 개인키는 로컬에 둔다. 공개키가 없거나 잘못되면 시작을 거부한다. 서버 host key는 기동 시 생성하므로 새 Pod의 지문을 별도로 확인한다.

이미지는 키 인증만 허용하고 SSH 포워딩 목적지를 `127.0.0.1:8000`으로 제한한다. 기본 이미지의 `EXPOSE 8000` 메타데이터가 상속될 수 있으므로 Runpod 설정에서 공개 포트가 **22/tcp 하나인지 직접 확인**한다. Dockerfile의 `EXPOSE 22`만으로 외부 포트 매핑 정책이 보장되지는 않는다. SSH 또는 vLLM이 종료되면 나머지 프로세스도 정리한다. 이 동작은 Pod 과금을 종료하는 기능이 아니다.
